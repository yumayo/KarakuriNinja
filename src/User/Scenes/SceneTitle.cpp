...
namespace User
{
        ...
        , isblackout( false )
    {
        ...
        start_se = &GData::FindAudio( "SE/titlestart.wav" );
        roadTexture( );
        ...
        iconangle = 0.0f;
        kunaiangle = 0.0f;
        haguruma.push_back( Haguruma( Vec2f( env.getWindowWidth( )*0.1f, env.getWindowHeight( )*0.1f ),
                                      Vec2f( 400, 400 ), 0.3f, 1.5f, 4.f, HagurumaType::LEFTHAGURUMA, false ) );
        marucount = 0;
    }
    ...
    void SceneTitle::drawGameStartGauge( )
    {
        if ( countend( t_[LOGOSLIDE] ) )
        {
            gl::pushModelView( );
            gl::translate( 130, env.getWindowHeight( ) - 250 );
            gameStartGauge.Draw( );
            gl::popModelView( );
        }
    }
