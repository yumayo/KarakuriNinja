        ...
        for ( int i = 0; i < 100; i++ ) {
            ...
            if ( !( ranking == ( i + 1 ) ) || delay_t >= 1.0f ) {
                if ( ( pos.y > -200 ) && ( pos.y < env.getWindowHeight( ) + 200 ) && icons[i].isscoredraw ) {
                    tables.Draw( std::to_string( icons[i].rank )+u8"位", pos + Vec2f( -icons[i].size.x / 2 + 80, -tables.BoundingBox( std::to_string( icons[i].rank ) ).getHeight( ) / 2.45F ), ColorA( 0, 0, 0, 1 ), User::Fonts::Mode::LEFTUP );
                }
                if ( ( pos.y > -200 ) && ( pos.y < env.getWindowHeight( ) + 200 ) && icons[i].isscoredraw ) {
    ...
    void SceneResult::drawMyIcon( )
    {
        if ( rankin == true ) {
            if ( ( camera_t >= 1.0f ) && ( delay_t < 1.0f ) ) {
                ...
                makimonotex2->disable( );
                gl::popModelView( );
                tables.Draw( std::to_string( icons[ranking - 1].rank ) + u8"位", pos + Vec2f( -icons[ranking - 1].size.x / 2 + 80, -tables.BoundingBox( std::to_string( icons[ranking - 1].rank ) ).getHeight( ) / 2.45F ),
